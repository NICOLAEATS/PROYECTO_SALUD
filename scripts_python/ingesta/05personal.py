...
psql_path = r"C:\Program Files\PostgreSQL\15\bin\psql.exe"  # 👈 Ajusta si tu versión es distinta
...
for archivo in os.listdir(ruta_csv):
    if archivo.lower().startswith("maestropersonal") and archivo.lower().endswith(".csv"):
        archivo_csv = os.path.join(ruta_csv, archivo)
        break

if not archivo_csv:
    print("❌ No se encontró un archivo que empiece con 'maestropersonal' y termine en '.csv'")
    exit()
