# DOCUMENTACIÓN COMPLETA DEL SISTEMA WEB - DIRESA CUSCO
...
| Túneles | Cloudflare Tunnel (acceso remoto temporal) |
...
├── db_config.py                         # Configuración BD cross-platform
├── run.py                               # Lanzador web
├── read_excel.py                        # Lector Excel
├── modulo_maestros.py                   # Módulo maestros desktop
│
├── proyecto_salud_cusco_web/            # Web app (Flask)
│   ├── app.py                           # ~4770 líneas - API REST + lógica
│   ├── config.py                        # Rutas y constantes web
│   ├── html_report_builder.py           # ~1664 líneas - Generador HTML MINSA
│   ├── analyze_excel.py                 # Analizador Excel
│   ├── requirements.txt                 # Dependencias Python
│   ├── templates/
│   │   └── index.html                   # SPA completa (todo el HTML)
│   ├── static/
│   │   ├── js/
│   │   │   ├── app.js                   # ~1340 líneas - Core SPA
│   │   │   └── modules.js              # ~1249 líneas - Módulos
│   │   └── css/
│   │       └── style.css                # ~1304 líneas - Estilos
│   ├── tunnel/                          # Scripts Cloudflare Tunnel
│   │   ├── start_tunnel.bat
│   │   ├── start_tunnel.ps1
│   │   └── TUNNEL_README.md
...
│   ├── ingesta/                         # Pipeline de datos
│   │   ├── 01cargacvs_universal.py      # Carga CSV completa
│   │   ├── 01cargacvs_mensual.py        # Carga CSV por mes
│   │   ├── 01cargacvs_hisminsa.py       # Carga HIS MINSA
│   │   ├── generar_his_proceso.py       # ETL principal (~1046 líneas)
│   │   ├── cargar_maestros.py           # Carga maestros
│   │   ├── procesar_eess_principal.py   # Procesa EESS
│   │   └── actualizar_his_proceso_maestros.py
│   ├── bi/                              # Business Intelligence
│   │   ├── cargar_padron_nominal.py     # Carga PN
│   │   ├── cargar_cnv.py               # Carga CNV
│   │   ├── generar_tabla_vacunas.py     # Genera tabla_vacunas
│   │   ├── generar_cred.py              # Genera tablas cred2024*
│   │   ├── generar_pai.py              # Genera tablas pai_2024
│   │   ├── 04_generador_reportes.py     # Ejecuta SQL SELECT
│   │   ├── 04_ejecutor_procedures.py    # Ejecuta SQL multi-statement
│   │   ├── selector_carpeta.py          # Diálogo nativo de carpeta
│   │   └── verificar_tablas.py          # Verificación
│   ├── mantenimiento/
│   │   └── 02_eliminar_datos.py         # Eliminación de datos
│   └── instalacion/
│       └── instalar_postgresql.py       # Instalación PostgreSQL
│
├── scripts_sql/                         # Scripts SQL
│   ├── SCRIPTS CORREGIDOS ULTIMOS/      # SQLs finales
│   │   ├── tabla vacunas cred_ivan.sql  # Tabla Vacunas/CRED
│   │   ├── tabla_materno_ivan.sql       # Tabla Materno
│   │   ├── tabla_iras_edas_ivan_2026.sql # Tabla IRAS/EDAS
│   │   ├── reporte_cred_ivan_2026.sql   # SP CRED (~6226 líneas)
│   │   └── reporte_iras_edas_2026_ivan.sql
│   ├── scripst tabla y reportes vacunas-cred/
│   ├── reportes/                        # Reportes SQL
│   ├── PADRONES/                        # SQLs de padrón
│   └── reportes exel/                   # HTMLs de referencia + Excel
│       ├── referencia de formato exel cred.txt
│       ├── referencia de foramto exel iras_edas.txt
│       ├── CRED FINAL 2026.xlsm
│       └── IRAS_EDAS 2026.xlsm
│
├── config/                              # Config persistente
│   └── db_connection.json               # Config BD guardada
│
└── proyecto-salud-pkg/                  # Paquete pacman Linux
```
...
ARCHIVOS CRUDOS (11_CUSCO_MM.zip/rar/7z)
  │  (mensuales por mes, contienen CSVs con datos HIS)
  ▼
EXTRACTOR (extractor_archivos.py, externo)
  │  (descompresión a carpeta temporal)
  ▼
CARGA CSV (01cargacvs_universal.py o 01cargacvs_mensual.py)
  │  Lee CSVs → staging table: es_ivan.hisminsa24
  │  (todos los archivos CSV del mes/año)
  ▼
CARGA MAESTROS (cargar_maestros.py)
  │  Tablas: maestro_paciente, maestro_personal, eess2025,
  │  maestro_his_cie_cpms, maestro_his_*, etc.
  ▼
GENERAR HIS PROCESO (generar_his_proceso.py)
  │  ETL principal: JOIN hisminsa24 + maestros
  │  → es_ivan.his_proceso_YYYY (tabla detalle)
  │  (~1,046 líneas, la transformación más compleja)
  ▼
TABLAS AGREGADAS:
  ├─ generar_tabla_vacunas.py → es_ivan.tabla_vacunas
  ├─ generar_cred.py          → es_ivan.cred2024* (5 tablas)
  │                               cred2024 (270 cols, crosstab)
  │                               cred2024_1 (267 cols)
  │                               cred2024_2 (34 cols)
  │                               cred2024_3 (163 cols)
  │                               cred2024_4 (174 cols)
  ├─ generar_pai.py           → es_ivan.pai_2024* (2 tablas)
  ├─ SQL Scripts              → es_ivan.tabla_materno
  │                              es_ivan.tabla_iras_edas
  └─ Cargas externas:
     ├─ cargar_padron_nominal.py → es_ivan.padron_nominal
     └─ cargar_cnv.py           → es_ivan.cnv_cusco
```

---

## 2. CONFIGURACIÓN DE BASE DE DATOS (db_config.py)

**Archivo**: `db_config.py` (~1476 líneas)
**Propósito**: Gestión centralizada de conexión PostgreSQL
...
| Contraseña | ivan | DB_PASSWORD |

### 2.2 Persistencia

- Config guardada en: `%APPDATA%/Proyecto_Salud_Cusco/config/db_connection.json`
- Formato JSON con todos los parámetros
- Lectura automática al iniciar la app
...
| `_detectar_postgresql_linux()` | Detección Linux: which psql, systemctl |
| `_pgpass_paths()` | Busca pgpass en rutas estándar |

### 2.4 Endpoints Web Relacionados

```
/api/db/config [GET, POST]     → Obtener/Guardar config
/api/db/detect [POST]          → Detectar PostgreSQL
/api/db/verify [POST]          → Verificar BD+esquema
/api/db/install [POST]         → Instalar PostgreSQL
/api/db/start-service [POST]   → Iniciar servicio PostgreSQL
/api/db/init [POST]           → Crear BD+esquema
/api/db/recover-password [POST] → Recuperar/crear base de datos
```
...
**Archivo**: `proyecto_salud_cusco_web/app.py` (~4770 líneas)
...
- Template único: `index.html` (SPA)
- Archivos estáticos: `/static/js/*`, `/static/css/*`
- Cache deshabilitado para HTML y JS (vía `@app.after_request`)
...
| `_init_state(token, nombre)` | 65 | Inicializa estado de ejecución |
| `_append_line(token, line)` | 80 | Agrega línea al log de ejecución |
...
| `_get_state(token)` | 118 | Obtiene estado actual de ejecución |
...
| `_cargar_config_editor()` | 140 | Carga configuración del editor SQL |
| `_guardar_config_editor(config)` | 147 | Guarda configuración del editor SQL |
| `ejecutar_script(ruta_script, args, mostrar_progreso)` | 152 | Ejecuta script Python/SQL en hilo separado con polling |
| `_db_cursor()` | 726 | Obtiene conexión PostgreSQL |
...
| `_borrar_anio_bd(anio)` | 436 | Borra registros de un año |
| `_resolve_year_table(cur, schema, template, year, fallback)` | 1099 | Resuelve tabla por año con fallback |
...
#### 3.3.3 Ejecución (2 endpoints)

| Método | Ruta | Función | Propósito |
|--------|------|---------|-----------|
| GET | /api/ejecucion/status/<token> | `ejecucion_status()` | Estado de ejecución (polling) |
| POST | /api/ejecucion/cancel | `ejecucion_cancel()` | Cancelar ejecución en curso |
...
| POST | /api/ingesta/delete | `ingesta_delete()` | Eliminar datos (todo/año/mes) |
...
| POST | /api/reportes/save-config | `reportes_save_config()` | Guardar configuración del editor |
...
#### 3.3.7 Padrón Nominal (5 endpoints)
...
| POST | /api/padron/consulta | `padron_consulta()` | Consultar PN con paginación/búsqueda |
...
| POST | /api/cnv/consulta | `cnv_consulta()` | Consultar CNV con paginación |

#### 3.3.9 Mapa / Geolocalización (4 endpoints)
...
| GET | /api/poblacion/resumen | `dash_poblacion()` | Resumen población/demografía |
...
| GET | /api/dashboards/vacunacion | `dash_vacunacion()` | Dashboard Vacunación |
| GET | /api/dashboards/cred | `dash_cred()` | Dashboard CRED |
| GET | /api/dashboards/suplementacion | `dash_suplementacion()` | Dashboard Suplementación |
| GET | /api/dashboards/materno | `dash_materno()` | Dashboard Materno |
| GET | /api/dashboards/poblacion | `dash_poblacion()` | Dashboard Población |
...
| GET | /api/tunnel/status | `tunnel_status()` | Estado del túnel |
| POST | /api/tunnel/detener | `tunnel_detener()` | Detener túnel |
...
| GET | /api/categorias | `categorias_list()` | Listar categorías |
...
**Archivos**:
- `templates/index.html` → Estructura HTML completa (todos los módulos)
- `static/js/app.js` → Core: navegación, polling, modales (~1340 líneas)
- `static/js/modules.js` → Módulos específicos: PN, CNV, Mapas, Dashboards, MINSA (~1249 líneas)
- `static/css/style.css` → Estilos con tema claro/oscuro (~1304 líneas)
...
### 4.3 Navegación (app.js)

```javascript
document.querySelectorAll('.nav-btn[data-module]')
// Módulos: bd, ingesta, reportes, maestros, consulta, editor,
//          dashboards, poblacion, mapa, minsa, tunnel
```

**Protección**: Los módulos ingesta/reportes/maestros requieren BD verificada (`S.bdOk`).
...
**Flujo**: POST → recibe token → polling 800ms → muestra líneas nuevas → barra progreso → detecta estados terminales → botón cancelar.

### 4.5 Módulos del Frontend (modules.js)

#### 4.5.1 Padrón Nominal (PN)
```javascript
var PN = { pagina: 1, total: 0, porPagina: 50 };
async function pnCargar()     → POST /api/padron/cargar
async function pnConsultar()  → POST /api/padron/consulta (paginación 50)
function pnRenderTabla(d)     → Renderiza tabla HTML
```

#### 4.5.2 CNV
```javascript
var CNV = { pagina: 1, total: 0, porPagina: 50 };
async function cnvCargar()    → POST /api/cnv/cargar
async function cnvConsultar() → POST /api/cnv/consulta
```

#### 4.5.3 Dashboard (6 dashboards)
```javascript
var dashCharts = {};
function dashCambiar(vista)   → Resumen, IRAS/EDAS, Vacunación, CRED, Suplementación, Materno, Población
...
```javascript
var MAPA = null;
function mapaInit()          → Inicializa mapa Leaflet
async function mapaKDE()     → POST /api/mapa/kde
```
...
async function rmInit()      → GET /api/reportes-minsa/filtros
async function rmEjecutar()  → POST /api/reportes-minsa/ejecutar
function rmRender(d)         → Renderiza resultado con pestañas
function rmRenderPagina(p)   → Renderiza página individual
function rmExportar()        → POST /api/reportes-minsa/exportar
```
...
**Archivo**: `proyecto_salud_cusco_web/html_report_builder.py` (~1664 líneas)
...
| `_fmt(v)` | 16 | Formateo de números |
| `_v(d, k)` | 25 | Valor desde dict con default 0 |
| `_num(v)` | 37 | TD con clase numérica |
| `_td(v, cls)` | 45 | TD genérico |
...
| `_sec_atencion_rn(c24, c1, c4)` | 169 | Atención del RN (dual-grid, 6 sub-secciones) |
| `_sec_sesiones(all_data)` | 315 | Sesiones de estimulación temprana |
| `_sec_lactancia(all_data)` | 337 | Lactancia materna (tabla 50% width) |
| `_sec_evaluacion_desarrollo(all_data)` | 284 | Evaluación del desarrollo (rowspan 3, 13 cols) |
| `_sec_plan_integral(all_data)` | 355 | Plan integral (13 edades, elaborado/ejecutado) |
| `_sec_consejeria(all_data)` | 387 | Consejería (10 items × 14 columnas) |
| `_sec_evaluacion_nutricional(all_data)` | 478 | Evaluación nutricional + IMC 5-11a + Parasitosis |
| `_sec_laboratorio(all_data)` | 555 | Laboratorio/tamizajes (5 grupos etarios × 3 cols) |
| `_sec_profilaxis(all_data)` | 604 | Profilaxis antiparasitaria (1ra/2da dosis) |
| `_sec_visita_domiciliaria(all_data)` | 629 | Visita domiciliaria (5 tipos × 7 edades) |
| `_sec_salud_mental(all_data)` | 662 | Salud mental (dual-grid: psicosocial + agudeza/postural/tanner) |
| `_sec_tamizajes(all_data)` | 731 | Tamizajes (violencia, depresión, alcohol, neurodesarrollo) |
| `_sec_tamizajes_positivos(all_data)` | 767 | Tamizajes positivos |
| `_sec_rop(all_data)` | 801 | ROP (12 actividades × 4 edades) |
| `_sec_salud_ocular_menor3(all_data)` | 842 | Salud ocular <3a (4 actividades × 6 edades) |
| `_sec_errores_refraccion(all_data)` | 870 | Errores refracción (3 categorías × 3 edades) |
| `_sec_iras_edas(all_data)` | 908 | IRAS/EDAS (IRA + SOB + EDA + Zinc/SRO con colgroups) |
| `build_page1_html(...)` | 1069 | Página 1 completa (6 secciones del ref) |
| `build_iras_edas_html(data)` | 1109 | IRAS/EDAS standalone (IRA con fórmulas, oxigenoterapia, SOB 7 edades, EDA 3 edades, Zinc, Resumen) |
| `build_suplementacion_html(secciones)` | 1645 | Página 2: Suplementación |
| `build_tx_anemia_html(secciones)` | 1656 | Página 3: Tratamiento Anemia |
| `_render_seccion_table(sec)` | 1602 | Renderiza sección genérica con CSS classes |

### 5.3 Estructura de Página 1 (CRED)
...
<div class="page">
  ...
  <div class="section-title">CONTROL DE CRECIMIENTO Y DESARROLLO DEL NIÑO(A)</div>
  <div class="sub-section-title">N° de Controles de Crecimiento y Desarrollo de 0 a 11 Años</div>
  <table> 13 columnas (GRUPO ETAREO + PROGRAMACIÓN + 11 controles) </table>

  <div class="section-title">I. ATENCIÓN DEL RECIÉN NACIDO</div>
  <div class="dual-grid">
    <div> Atención Inmediata + Condición Nacimiento + Tamizaje Neonatal </div>
    <div> Alojamiento Conjunto + Consejería + Visita Domiciliaria </div>
  </div>

  <div class="section-title">IX. EVALUACIÓN DEL DESARROLLO</div>
  <table> (header con rowspan=3, 13 cols) </table>

  <div class="dual-grid">
    <div class="sub-section-title">II. SESIONES DE ATENCIÓN TEMPRANA</div>
    <table>
    <div class="section-title">VI. LACTANCIA MATERNA EXCLUSIVA</div>
    <table style="width:50%">
  </div>

  <div class="section-title">XVI. ATENCIÓN DE LAS ENFERMEDADES PREVALENTES DE LA INFANCIA</div>
  <table> IRA + SOB/Asma + EDA + Zinc/SRO </table>
</div>
```

### 5.4 Archivos de Referencia (guía exacta)

Ubicación: `scripts_sql/reportes/reportes exel/`

1. **`referencia de formato exel cred.txt`** (566 líneas)
   - HTML completo del reporte CRED
   - CSS completo + estructura de 6 secciones
   - Colgroups: 14%+9%+7%×11 (Control CRED)

2. **`referencia de foramto exel iras_edas.txt`** (678 líneas)
   - HTML completo del reporte IRAS/EDAS
   - Secciones: IRA (9 cols con fórmulas), Oxigenoterapia, SOB/Asma (10 cols), EDA (6 cols), Zinc/SRO, Resumen
...
1. Recibe: año (y opcionalmente meses/ruta carpeta)
2. Busca archivos CSV (patrón: `*MM*.csv`, `11_CUSCO_MM*.csv`, etc.)
...
### 6.3 Generación HIS Proceso (ETL Core)

**Script**: `generar_his_proceso.py` (~1046 líneas)
- JOIN hisminsa24 + maestros → `his_proceso_YYYY`

### 6.4 Generación de Tablas Agregadas
...
### 6.5 Carga Padrón Nominal
**Script**: `cargar_padron_nominal.py` → 347,265 registros

### 6.6 Carga CNV
**Script**: `cargar_cnv.py` → 257,019 registros

---

## 7. EJECUCIÓN DE SCRIPTS (Polling)

### 7.1 Estado Compartido Thread-Safe

```python
_exec_state = {}              # dict(token → state)
...
| GET | /api/tunnel/status | Estado del túnel |
| POST | /api/tunnel/detener | Detiene el túnel |
...
| Tabla | Tipo | Filas | Propósito |
|-------|------|-------|-----------|
| `hisminsa24` | Staging | Variable | Datos crudos de importación CSV |
| `his_proceso_YYYY` | Detalle | Millones | Datos enriquecidos por año |
| `padron_nominal` | Maestro | 347,265 | Padrón nominal de niños |
| `cnv_cusco` | Maestro | 257,019 | Certificados de nacido vivo |
| `maestro_*` (22 tablas) | Maestro | Variable | Catálogos del sistema |
| `eess2025` | Maestro | ~400 | Establecimientos de salud |
| `tabla_vacunas` | Agregada | 13,108,188 | Vacunación |
| `tabla_materno` | Agregada | 1,531,528 | Atención materna |
...
- **Oracle Cloud Free Tier** (gratis siempre): En proceso de creación
- **Hetzner CX22** (€3.99/mes): Plan B
- Tunnel Cloudflare para acceso remoto temporal

---

## 11. CONFIGURACIÓN CRUZADA (app.py ↔ frontend)

### 11.1 Versiones de Archivos Estáticos
...
2. Ir a "Configurar BD" → Detectar → Verificar → Iniciar BD
3. Ir a "Ingesta" → Seleccionar carpeta con CSVs → Importar (modo reemplazar)
4. Ir a "Procesar" → Cargar maestros → Generar HIS Proceso
5. Ir a "Reportes" → Generar tablas agregadas
6. Ir a "Formatos MINSA" → Seleccionar año/meses → Generar reporte
7. Usar Dashboards para visualización rápida
8. Usar Mapa para geolocalización

### Usuario existente (solo actualizar datos):
1. Ir a "Ingesta" → Importar nuevos CSVs (modo completar)
2. Ir a "Procesar" → Refrescar HIS proceso
3. Ir a "Reportes" → Regenerar tablas agregadas
4. Ir a "Formatos MINSA" → Generar reporte actualizado

---

## 14. DIAGNÓSTICO DE ERRORES DEL MÓDULO FORMATOS MINSA

### 14.1 LISTA COMPLETA DE ERRORES CONOCIDOS (CRÍTICOS Y MENORES)

A continuación se enumeran **todos los errores** identificados en el módulo Formatos MINSA al momento de esta documentación. Se clasifican por gravedad y se detalla la causa raíz, el archivo/ línea y la solución.

---

### 14.2 ERRORES BLOQUEANTES (CRÍTICOS)

#### E01. Error de sintaxis JS: `/*` sin `*/` en modules.js

**Archivo**: `static/js/modules.js`
**Línea**: ~908 (entre dashPoblacion y el resto del código)
**Síntoma**: Todo el JavaScript después del bloque `/*` abierto dejaba de ejecutarse. Los módulos Mapa, Dashboards y Formatos MINSA no funcionaban. El error se ve en consola del navegador como `Uncaught SyntaxError: Unexpected end of input`.
**Causa**: Un bloque de comentario `/* ... */` se abrió pero nunca se cerró con `*/`.
**Solución**: Cerrar el comentario con `*/` antes del `catch` o eliminar el bloque.
**Estado**: CORREGIDO.

#### E02. `tabla_html` sin `<style>` en años ≠ 2024

**Archivo**: `app.py` en `_reporte_completo_cred_old()`
**Línea**: ~3894
**Síntoma**: Para años 2025/2026, el HTML generado contenía las clases CSS (`.section-title`, `.th-dark`, etc.) pero NO incluía el `<style>` con las reglas CSS. El navegador no aplicaba los estilos y el reporte se veía como HTML plano (texto negro sobre fondo blanco, sin bordes, sin colores).
**Causa**: El loop que genera `tabla_html` para años viejos no anteponía `'<style>' + _REPORT_CSS + '</style>'` al HTML.
**Solución**: Agregar `'<style>' + _REPORT_CSS + '</style>'` al inicio del `tabla_html` generado en el loop de páginas.
**Estado**: CORREGIDO.

#### E03. Columnas dinámicas en `_sec_cred_controls` no coinciden con el ref

**Archivo**: `html_report_builder.py` en `_sec_cred_controls()`
**Línea**: 128-164
**Síntoma**: Originalmente usaba `col_names[2:]` para determinar las columnas de controles, que producía un número variable de columnas. El ref HTML exige exactamente 13 columnas fijas: GRUPO ETAREO + PROGRAMACIÓN + 11 controles (1er Ctrl 29-59d ... 11vo Ctrl).
**Causa**: El código original generaba columnas dinámicas desde la consulta BD.
**Solución**: Reemplazar con 13 columnas fijas usando `ctrl_names` y `ctrl_keys` hardcodeados.
**Estado**: CORREGIDO.

#### E04. Colgroups faltantes en `_sec_cred_controls` e `_sec_iras_edas`

**Archivo**: `html_report_builder.py`
**Líneas**: 128-164 y 908-1064
**Síntoma**: Las tablas no tenían `<colgroup>` o tenían widths incorrectos. El ref HTML especifica widths exactos: 14%+9%+7%×11 para Control CRED; 28%+10%×6+12% para IRA/SOB; 28%+11%×6+12% para EDA/Zinc.
**Causa**: El builder original no incluía `<colgroup>`.
**Solución**: Agregar `<colgroup>` con los % exactos del ref.
**Estado**: CORREGIDO.
...
**Línea**: 100-123
**Síntoma**: El header tenía dos filas de filtros pero ambas usaban `class="filter-row"`. El ref HTML usa `.filter-row` (4 columnas, border-top + border-bottom) y `.filter-row2` (5 columnas, solo border-bottom).
**Causa**: Código original usaba `filter-row` para ambas.
**Solución**: Cambiar la segunda fila a `class="filter-row2"` con 5 columnas (última vacía).
**Estado**: CORREGIDO.
...
**Línea**: ~262
**Síntoma**: La sección "E) Atención del Recién Nacido en Visita Domiciliaria" tenía header `ACTIVIDADES` pero el ref HTML dice `DIAGNÓSTICOS`.
**Causa**: Copia incorrecta del label del ref.
**Solución**: Cambiar "ACTIVIDADES" a "DIAGNÓSTICOS".
**Estado**: CORREGIDO.

#### E07. Secciones extra en Página 1 que no están en el ref

**Archivo**: `html_report_builder.py` en `build_page1_html()`
**Línea**: ~1069-1103
**Síntoma**: La página 1 incluía 11 secciones extras: Plan Integral, Consejería, Evaluación Nutricional, Laboratorio/ Tamizajes, Profilaxis Antiparasitaria, Visita Domiciliaria, Salud Mental, Tamizajes, Tamizajes Positivos, ROP, Salud Ocular, Errores Refracción. El ref HTML solo tiene 6 secciones: Control CRED, Atención RN, Evaluación Desarrollo, Sesiones, Lactancia, IRAS/EDAS.
**Causa**: El builder original construía todas las secciones del Excel completo en página 1.
**Solución**: Eliminar las secciones extras de `build_page1_html()`.
**Estado**: CORREGIDO.
...
**Archivo**: `html_report_builder.py` en `build_page1_html()`
**Línea**: ~1088-1092
**Síntoma**: Sesiones II y Lactancia VI se renderizaban en filas separadas (una debajo de otra), pero el ref HTML y el Excel las muestran lado a lado.
**Causa**: Faltaba envolverlas en `<div class="dual-grid">`.
**Solución**: Envolver ambas secciones en `<div class="dual-grid">`.
**Estado**: CORREGIDO.
...
**Línea**: 1190-1202
**Síntoma**: El contenido del reporte se desbordaba del contenedor sin scroll horizontal.
**Causa**: `.page` no tenía `overflow: auto`.
**Solución**: Agregar `overflow: auto` a `.page`.
**Estado**: CORREGIDO.
...
**Línea**: 315-331
**Síntoma**: La tabla de sesiones de atención temprana solo tiene una fila (RN). Debería tener múltiples filas para diferentes edades con sesiones 1-5 y columna de "completas (Mensual)" y "completas (Acum.)".
**Causa**: El ref HTML muestra una estructura más compleja con varias filas etarias.
**Solución pendiente**: Implementar las filas restantes según el ref HTML.

#### E11. `_sec_evaluacion_desarrollo` tiene fórmula hardcodeada "TD=..." en la última columna

**Archivo**: `html_report_builder.py` en `_sec_evaluacion_desarrollo()`
**Línea**: 306
**Síntoma**: En vez de mostrar un valor numérico real de "Evaluación Normal", muestra un texto de fórmula "TD=D+DX=Z006+LAB=ED".
**Causa**: Falta la query BD para obtener el valor real de evaluaciones normales.
**Solución pendiente**: Agregar consulta BD y reemplazar la fórmula hardcodeada.

#### E12. `_sec_sesiones` y `_sec_lactancia` muestran datos inconsistentes

**Archivo**: `html_report_builder.py`
**Síntoma**: Ambas secciones usan `all_data.get(...)` con keys que pueden no existir en `data_composite` porque los datos vienen de tablas diferentes (cred2024 vs his_proceso). Cuando no hay datos, muestran 0.
**Causa**: `data_composite` se construye mergeando todas las columnas de cred2024* pero las keys pueden diferir.
**Solución**: Verificar que las keys en `_sec_sesiones` y `_sec_lactancia` coincidan con las columnas reales de las tablas.

#### E13. `tabla_html` puede no estar presente en la respuesta para 2024

**Archivo**: `app.py` en `_reporte_cred_2024()`
**Línea**: ~2833-3493
**Síntoma**: Para 2024, la respuesta incluye `tabla_html` solo si el builder se ejecuta correctamente. Si hay un error en el builder (como un KeyError por columna faltante), se cae al `except` y usa `_reporte_completo_cred_old()`, que anteriormente no generaba `tabla_html`.
**Causa**: El `try/except` en `_reporte_completo_cred()` captura cualquier error del builder.
**Solución**: Asegurar que tanto 2024 como old siempre generen `tabla_html`.

#### E14. Exportación a Excel puede fallar si faltan columnas

**Archivo**: `app.py` en `reportes_minsa_exportar()`
**Síntoma**: La exportación a Excel usa `openpyxl` directamente en memoria. Si la estructura de `secciones` o `filas` cambia, la exportación puede fallar con KeyError.
**Estado**: CONFIRMADO como funcional en pruebas anteriores, pero frágil ante cambios de estructura.

#### E15. Respuesta `/api/reportes-minsa/ejecutar` no tiene cache busting

**Síntoma**: Flask tiene debug mode off y los cambios en `modules.js` pueden no reflejarse si el navegador cachea el archivo.
**Solución**: Incrementar `v=` en `index.html` para forzar recarga de JS.

---

### 14.4 ERRORES DE VISUALIZACIÓN (MENORES)

#### E16. `modules.js` no tiene `rmRenderSeccion` ni `rmRenderPagina` correctas para el nuevo formato HTML con CSS classes

**Archivo**: `static/js/modules.js`
**Líneas**: 1088-1123
**Síntoma**: `rmRenderPagina()` tiene un `if (p.tabla_html && p.tabla_html.length > 0)` que renderiza directamente el HTML. Pero si `tabla_html` está vacío o es `undefined`, cae al código legacy que usa `columnas`/`filas`/`secciones` con inline styles. El código legacy **nunca** genera el nuevo formato de CSS classes.
**Causa**: El builder ahora genera `tabla_html` para todas las rutas, pero el código legacy se mantiene como fallback.
**Estado**: El código legacy es código muerto si `tabla_html` siempre se genera.

#### E17. `.filter-row2` tiene grid de 5 columnas pero la última está vacía

**Archivo**: `html_report_builder.py` en `_sec_header()` y `static/css/style.css`
**Síntoma**: La última celda de `.filter-row2` es un `<div class="filter-cell"></div>` vacío. Esto es intencional para mantener 5 columnas, pero en el ref HTML esa celda no existe (el ref usa 4 columnas en ambas filas).
**Causa**: Se agregó una 5ta columna vacía para que el layout coincida visualmente con el ref.
**Estado**: Intencional, pero puede causar problemas de layout responsivo.

#### E18. `dual-grid` no tiene comportamiento responsivo

**Archivo**: `static/css/style.css` línea 1288-1293
**Síntoma**: `.dual-grid` usa `grid-template-columns: 1fr 1fr` fijo. En pantallas pequeñas (<768px) las dos columnas se comprimen demasiado.
**Causa**: No hay media query que cambie a 1 columna.
**Solución**: Agregar `@media (max-width: 768px) { .dual-grid { grid-template-columns: 1fr; } }`.
**Estado**: NO CORREGIDO.
...
**Línea**: 3962
**Síntoma**: La función construye `tbl = f'es_ivan.his_proceso_{anio}'` pero solo se llama desde `_reporte_iras_edas_2024()` con `anio=2024` hardcodeado.
...
**Línea**: 3929
**Síntoma**: `LOWER({col}) LIKE LOWER('%{val}%')` con interpolación directa. Aunque `val` reemplaza `'`, es una práctica insegura.
**Estado**: Riesgo bajo porque solo se usa con valores de la BD (categorías existentes), pero anticuado.

#### E21. `_reporte_cred_2024()` no usa la función `_build_cred_where()` correctamente

**Archivo**: `app.py`
**Línea**: 2835
**Síntoma**: `where = _build_cred_where(2024, meses, filtros)` hardcodea 2024.
...
#### E22. Doble ruteo (2024 vs old) es frágil y duplica lógica

**Archivo**: `app.py` en `_reporte_completo_cred()` y `_reporte_iras_edas()`
**Líneas**: 3907 y 4380
**Síntoma**: Dos implementaciones completamente separadas para el mismo reporte (una para 2024 usando tablas pre-agregadas, otra para otros años usando his_proceso). Esto duplica ~1500 líneas de lógica y hace que cualquier cambio deba aplicarse en ambos lados.
**Causa**: Las tablas pre-agregadas `cred2024*` solo existen para 2024. Para años diferentes se debe usar `his_proceso_YYYY`.
**Solución propuesta**: Unificar ambas rutas consultando directamente las tablas pre-agregadas cuando existan y haciendo fallback a his_proceso. O generar tablas agregadas dinámicamente para el año solicitado.

#### E23. `_reporte_completo_cred_old()` usa `_qflat()` que tiene SQL injection potencial

**Archivo**: `app.py`
**Línea**: 3506-3546
**Síntoma**: `codes_str = "','".join(codes)` y `f"WHERE codigo_item IN ('{cs}')"`.
**Estado**: Códigos controlados (CRED_CODES es constante), pero inseguro en general.

#### E24. `data_composite` mergea 5 tablas sin prefijos únicos

**Archivo**: `app.py` en `_reporte_cred_2024()`
**Síntoma**: El merge de `{**c24, **c1, **c2, **c3, **c4}` puede sobreescribir keys si hay columnas con mismo nombre en diferentes tablas.
**Solución**: Usar prefijos C1_, C2_, C3_, C4_ que ya existen en las tablas.

#### E25. `config.py` puede no detectar correctamente PROJECT_ROOT en Windows

**Archivo**: `config.py`
**Síntoma**: Busca `scripts_python/` y `proyecto_salud_cusco_web/` en el árbol de directorios ascendente. En algunas configuraciones de Windows con rutas con espacios, la detección puede fallar.
**Solución**: Usar `os.environ.get('PROYECTO_SALUD_ROOT')` como fallback.

---

### 14.7 DIAGRAMA DE FLUJO DEL MÓDULO FORMATOS MINSA

```
USUARIO: 
  1. Navega a "Formatos MINSA"
  2. rmInit() → GET /api/reportes-minsa/filtros
     → Puebla selects: año, red, microred, establecimiento, provincia, distrito
  3. Selecciona año, meses, filtros
  4. rmEjecutar() → POST /api/reportes-minsa/ejecutar
     ↓
app.py: reportes_minsa_ejecutar()
  ├─ tipo = body['tipo']  ('formato_nino_cred' | 'formato_iras_edas')
  ├─ anio = body['anio']
  ├─ meses = body['meses']
  ├─ filtros = body[...]
  │
  ├─ if tipo == 'formato_nino_cred':
  │   └─ _reporte_completo_cred(cur, esquema, anio, meses, filtros)
  │       ├─ if anio == '2024':
  │       │   └─ _reporte_cred_2024(cur, esquema, anio, meses, filtros)
  │       │       ├─ _qcred_cols() × 5 (c24, c1, c2, c3, c4)
  │       │       ├─ _build_cred_where(2024, meses, filtros)
  │       │       ├─ Construye 'secciones' (formato dict legacy)
  │       │       ├─ build_page1_html() → tabla_html CON <style>
  │       │       ├─ build_suplementacion_html() → tabla_html
  │       │       └─ build_tx_anemia_html() → tabla_html
  │       │
  │       └─ else (2025/2026):
  │           └─ _reporte_completo_cred_old(cur, esquema, anio, meses, filtros)
  │               ├─ CRED_CODES, ATENCION_RN_CODES, etc.
  │               ├─ _qflat(), _qval(), _qmonth_pivot()
  │               ├─ Construye 'secciones' (formato dict legacy)
  │               └─ Genera tabla_html con loop inline + <style>
  │
  ├─ if tipo == 'formato_iras_edas':
  │   └─ _reporte_iras_edas(cur, esquema, anio, meses, filtros)
  │       ├─ if anio == '2024':
  │       │   └─ _reporte_iras_edas_2024()
  │       │       ├─ _qcred_cols(cur, {'IE':'iras_edas_2024'}, where)
  │       │       ├─ _query_iras_ref(cur, 2024, filtros) → diag
  │       │       ├─ _query_age_groups(cur, 2024, CPT codes) → proc
  │       │       ├─ _query_resumen_mensual(cur, 2024) → resumen
  │       │       └─ build_iras_edas_html() → tabla_html
  │       └─ else (2025/2026):
  │           └─ _reporte_iras_edas_old()
  │               ├─ CIE_IRAS, CIE_SOB_ASMA, CIE_EDAS, etc.
  │               ├─ _query_cie() para cada grupo
  │               └─ Construye 'secciones' (formato dict legacy)
  │
  └─ Retorna { tipo, anio, meses, filtros, paginas: [{tabla_html, ...}] }
     ↓
FRONTEND: rmRender(d)
  ├─ Crea tabs por página
  ├─ Extrae CSS de tabla_html[0] → inyecta en <head>
  └─ rmRenderPagina(p)
      ├─ if p.tabla_html: renderiza DIRECTAMENTE
      └─ else: renderiza legacy (columnas/filas/secciones con inline styles)
```

### 14.8 MAPA DE ARCHIVOS DEL MÓDULO FORMATOS MINSA

| Archivo | Líneas | Rol |
|---------|--------|-----|
| `app.py:2576` | reportes_minsa_ejecutar() | Endpoint POST principal del módulo |
...
| `app.py:3907-3916` | _reporte_completo_cred() | Router CRED (2024 → nuevo, else → old) |
...
| `app.py:4215-4377` | _build_cred_suplementacion(), _build_cred_tx_anemia() | Constructores de secciones Suplementación y Tx Anemia |
| `app.py:4380-4389` | _reporte_iras_edas() | Router IRAS/EDAS (2024 → nuevo, else → old) |
...
| `html_report_builder.py:169-279` | _sec_atencion_rn() | Atención RN (dual-grid) |
| `html_report_builder.py:284-310` | _sec_evaluacion_desarrollo() | Evaluación Desarrollo |
| `html_report_builder.py:315-332` | _sec_sesiones() | Sesiones Atención Temprana |
...
| `html_report_builder.py:387-473` | _sec_consejeria() | Consejería |
| `html_report_builder.py:478-550` | _sec_evaluacion_nutricional() | Evaluación Nutricional + IMC + Parasitosis |
...
| `html_report_builder.py:870-903` | _sec_errores_refraccion() | Errores Refracción |
| `html_report_builder.py:908-1064` | _sec_iras_edas() | IRAS/EDAS |
| `html_report_builder.py:1069-1103` | build_page1_html() | Página 1 builder |
| `html_report_builder.py:1109-1599` | build_iras_edas_html() | IRAS/EDAS standalone builder |
| `html_report_builder.py:1602-1642` | _render_seccion_table() | Renderizador de sección genérica |
| `html_report_builder.py:1645-1653` | build_suplementacion_html() | Página 2 builder |
| `html_report_builder.py:1656-1664` | build_tx_anemia_html() | Página 3 builder |
...
### 14.9 CONSTANTES Y DATOS CRÍTICOS (app.py)

#### Constantes de códigos CRED
```python
CRED_CODES = [
...
#### Constantes de códigos Atención RN
```python
...
CIE_IRAS = {
    ...
    'Neumonía sin complicaciones': {'codes': ['J129','J159','J189']},
}
...
- `eess` (VARCHAR) - Código del establecimiento
...
### 14.11 TABLA DE CORRESPONDENCIA REF HTML ↔ BUILDER

| Sección del Ref | Función Builder | Línea | Estado |
|----------------|----------------|-------|--------|
| Header (.filter-row + .filter-row2) | `_sec_header()` | 100 | ✅ OK |
| Control CRED (13 cols fijas) | `_sec_cred_controls()` | 128 | ✅ OK |
| I. Atención RN (dual-grid) | `_sec_atencion_rn()` | 169 | ✅ OK |
| IX. Evaluación Desarrollo | `_sec_evaluacion_desarrollo()` | 284 | ⚠️ Fórmula hardcodeada |
| II. Sesiones Atención Temprana | `_sec_sesiones()` | 315 | ⚠️ Solo RN |
| VI. Lactancia Materna | `_sec_lactancia()` | 337 | ✅ OK |
| XVI. IRAS/EDAS | `_sec_iras_edas()` | 908 | ✅ OK |
| Suplementación (Page 2) | `build_suplementacion_html()` | 1645 | ✅ OK |
| Tx Anemia (Page 3) | `build_tx_anemia_html()` | 1656 | ✅ OK |
...
## 16. CHANGELOG DE CORRECCIONES DEL MÓDULO FORMATOS MINSA

| Fecha | Issue | Archivo | Cambio |
|-------|-------|---------|--------|
| 2026-06-22 | E01 | `modules.js` | Cerrado `/*` sin `*/` |
| 2026-06-22 | E02 | `app.py:3894` | Agregado `<style>` a tabla_html de años ≠ 2024 |
| 2026-06-22 | E03 | `html_report_builder.py:128` | 13 columnas fijas en vez de dinámicas |
...
| 2026-06-22 | E06 | `html_report_builder.py:262` | "ACTIVIDADES" → "DIAGNÓSTICOS" |
...
| 2026-06-22 | DOC | `DOCUMENTACION_SISTEMA_WEB.md` | Documentación completa creada |

---

## 17. PRIORIDAD DE CORRECCIONES PENDIENTES

| Prioridad | Issue | Descripción | Impacto |
|-----------|-------|-------------|---------|
| 🔴 Alta | E10 | `_sec_sesiones` incompleta (solo RN) | Faltan datos de sesiones para edades > RN |
| 🔴 Alta | E11 | `_sec_evaluacion_desarrollo` fórmula hardcodeada | Muestra fórmula en vez de datos reales |
| 🟡 Media | E12 | Keys inconsistentes entre builder y datos | Posibles ceros donde debería haber datos |
| 🟡 Media | E22 | Doble ruteo frágil (2024 vs old) | ~1500 líneas duplicadas, cambios deben aplicarse dos veces |
| 🟢 Baja | E18 | dual-grid no responsivo | Mala visualización en pantallas <768px |
| 🟢 Baja | E17 | filter-row2 columna vacía | Leve, solo afecta layout responsivo |
| 🟢 Baja | E16 | Código legacy muerto en rmRenderPagina | No afecta funcionalidad actual |

---

*Documentación generada del código fuente. Sistema de Monitoreo de Salud - GERESA Cusco (DIRESA CUSCO)*
...
## 18. C�DIGO FUENTE COMPLETO - html_report_builder.py
...
def _sec_atencion_rn(c24, c1, c4):
    ...
    # LEFT: B) Condición de Nacimiento
...
# ============================================================
# SECTION: IX. EVALUACIÓN DEL DESARROLLO
# ============================================================
...
# ============================================================
# SECTION: II. SESIONES DE ATENCIÓN TEMPRANA
# ============================================================
...
# ============================================================
# SECTION: X. PLAN DE ATENCIÓN INTEGRAL
# ============================================================
...
# ============================================================
# SECTION: V. EVALUACIÓN NUTRICIONAL
# ============================================================
...
# ============================================================
# SECTION: ATENCIÓN SALUD OCULAR < 3a
# ============================================================
...
# ============================================================
# SECTION: XVI. IRAS/EDAS (within FORMATO NIÑO)
# ============================================================
...
def build_iras_edas_html(data):
    ...
    # ================================================================
    # B. SÍNDROME DE OBSTRUCCIÓN BRONQUIAL (SOB) - ASMA
    # ================================================================
...
## 19. C�DIGO FUENTE COMPLETO - app.py (M�dulo Formatos MINSA)
...
## 20. C�DIGO FUENTE COMPLETO - modules.js (M�dulo Formatos MINSA)
...
## 21. C�DIGO FUENTE COMPLETO - style.css (Estilos Reporte MINSA)
...
## 22. REFERENCIA HTML - formato exel cred.txt
...
<title>CRED - Informe Mensual del Niño(a) 2026 - DIRESA CUSCO</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
...
<div class="page">
  ...
  <div class="filter-row">
    ...
    <div class="filter-cell"><label>AÑO:</label><span>2026</span></div>
  </div>
  <!-- Filters row 2 -->
  <div class="filter-row2">
  ...
  <table>
    ...
    <thead>
      <tr>
        ...
        <th class="th-dark">PROGRAMACIÓN</th>
    ...
    <tbody>
      <tr><td class="label-left">Recién Nacido Sano 3-6d</td><td></td><td class="num">5,256</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr class="row-sub"><td class="label-left">Recién Nacido Sano 7-13d</td><td></td><td class="num">278</td><td class="num">5,100</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr><td class="label-left">Recién Nacido Sano 14-21d</td><td></td><td class="num">58</td><td class="num">479</td><td class="num">5,006</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr class="row-sub"><td class="label-left">Recién Nacido Sano may_22d</td><td></td><td></td><td></td><td></td><td class="num">50</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      ...
      <tr><td class="label-left">&lt; 1 año</td><td></td><td class="num">5,349</td><td class="num">4,768</td><td class="num">4,807</td><td class="num">5,126</td><td class="num">4,963</td><td class="num">4,682</td><td class="num">4,737</td><td class="num">84</td><td class="num">111</td><td class="num">88</td><td class="num">124</td></tr>
      <tr class="row-sub"><td class="label-left">1 año</td><td></td><td class="num">4,951</td><td class="num">4,067</td><td class="num">3,742</td><td class="num">3,222</td><td class="num">94</td><td class="num">145</td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr><td class="label-left">2 años</td><td></td><td class="num">4,570</td><td class="num">3,498</td><td class="num">107</td><td class="num">122</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr class="row-sub"><td class="label-left">3 años</td><td></td><td class="num">4,320</td><td class="num">2,649</td><td class="num">84</td><td class="num">99</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr><td class="label-left">4 años</td><td></td><td class="num">3,693</td><td class="num">1,683</td><td class="num">57</td><td class="num">37</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr class="row-sub"><td class="label-left">5-9 años</td><td></td><td class="num">493</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
      <tr><td class="label-left">10-11 años</td><td></td><td class="num">94</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
  ...
  <div class="dual-grid">
    ...
    <div>
      <div class="sub2-title">A) Atención Inmediata</div>
      <table>
        <thead>
          <tr><th class="th-dark">ACTIVIDADES</th><th class="th-dark">N°</th></tr>
        </thead>
        <tbody>
          <tr><td class="label-left">Atención Inmediata</td><td class="num">5,360</td></tr>
          <tr class="row-sub"><td class="label-left">Corte tardío del Cordón Umbilical</td><td class="num">6,967</td></tr>
          <tr><td class="label-left">Contacto Piel a Piel con la madre</td><td class="num">5,246</td></tr>
          <tr class="row-sub"><td class="label-left">Examen físico del recién nacido normal</td><td class="num">7,929</td></tr>
          <tr><td class="label-left">Lactancia Materna en la 1ª Hora</td><td class="num">6,967</td></tr>
      ...
      <div class="sub2-title" style="margin-top:4px;">B) Condición de Nacimiento del Recién Nacido</div>
      <table>
        <thead>
          <tr><th class="th-dark">DIAGNÓSTICOS</th><th class="th-dark">N°</th></tr>
        </thead>
        <tbody>
          ...
          <tr class="row-sub"><td class="label-left">Macrosómico</td><td>106</td></tr>
          <tr><td class="label-left">Microcefalia</td><td class="zero">0</td></tr>
          <tr class="row-sub"><td class="label-left">Recién nacido prematuro</td><td>493</td></tr>
          <tr><td class="label-left">Recién Nacido Normal</td><td>45</td></tr>
        </tbody>
      </table>
      ...
      <table>
        ...
        <tbody>
          <tr><td class="label-left">Hipotiroidismo Congénito</td><td>4</td></tr>
          <tr class="row-sub"><td class="label-left">Fenilcetonuria Clásica</td><td>2</td></tr>
          <tr><td class="label-left">Hiperplasia Suprarrenal Congénita</td><td>16</td></tr>
          <tr class="row-sub"><td class="label-left">Tamizaje de Cardiopatía Congénita</td><td class="num">1,676</td></tr>
          <tr><td class="label-left">Fibrosis Quística, sin otra especificación</td><td>45</td></tr>
          <tr class="row-sub"><td class="label-left">Catarata Congénita</td><td>341</td></tr>
          <tr><td class="label-left">Cardiopatía congénita</td><td class="zero">0</td></tr>
    ...
    <div>
      <div class="sub2-title">C) Atención de Recién Nacido en Alojamiento Conjunto</div>
      <table>
        <thead>
          <tr><th class="th-dark">ACTIVIDADES</th><th class="th-dark">N°</th></tr>
        </thead>
        <tbody>
          <tr><td class="label-left">Atención del RN en Alojamiento Conjunto</td><td class="num">7,870</td></tr>
          <tr class="row-sub"><td class="label-left">Evaluación médica del recién nacido</td><td class="num">1,448</td></tr>
          ...
          <tr><td class="label-left">Tamizaje de catarata congénita</td><td>341</td></tr>
          <tr class="row-sub"><td class="label-left">Tamizaje de cardiopatía congénita</td><td class="num">1,676</td></tr>
        </tbody>
      </table>

      <div class="sub2-title" style="margin-top:4px;">Consejería en Atención del RN - Alojamiento Conjunto</div>
      <table>
        <thead>
          <tr><th class="th-dark">ACTIVIDADES</th><th class="th-dark">N°</th></tr>
        </thead>
        <tbody>
          <tr><td class="label-left">Consejería en corte y cuidado del cordón umbilical</td><td class="num">17,204</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería en Lactancia Materna Exclusiva</td><td class="num">33,274</td></tr>
          <tr><td class="label-left">Consejería en importancia del control CRED (4 controles)</td><td class="num">4,751</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería de identificación de signos de alarma</td><td class="num">11,870</td></tr>
          <tr><td class="label-left">Consejería en higiene del RN y cuidado en el hogar</td><td class="zero">0</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería en alimentación con sucedáneos (neonatos VIH)</td><td>55</td></tr>
        </tbody>
      </table>

      <div class="sub2-title" style="margin-top:4px;">E) Atención del Recién Nacido en Visita Domiciliaria</div>
      <table>
        <thead>
          <tr><th class="th-dark">DIAGNÓSTICOS</th><th class="th-dark">N°</th></tr>
        </thead>
        <tbody>
          <tr><td class="label-left">Visita domiciliaria para el cuidado y evaluación neonatal</td><td class="num">10,705</td></tr>
          <tr class="row-sub"><td class="label-left">Anamnesis y examen físico del RN normal</td><td class="num">16,156</td></tr>
          <tr><td class="label-left">Consejería en higiene del RN y cuidado en el hogar</td><td class="num">8,227</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería en cuidado del cordón umbilical</td><td class="num">28,253</td></tr>
          <tr><td class="label-left">Consejería en importancia del control CRED (4 controles)</td><td class="num">15,367</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería de identificación de signos de alarma</td><td class="num">31,091</td></tr>
          <tr><td class="label-left">Consejería en higiene de manos</td><td class="num">36,676</td></tr>
          <tr class="row-sub"><td class="label-left">Consejería en Lactancia Materna Exclusiva hasta 6 meses</td><td class="num">54,640</td></tr>
        </tbody>
      </table>
    </div>
  </div>
  ...
  <table>
    <thead>
      ...
      <tr>
        ...
        <th class="th-medium" colspan="2">Coordinación</th>
    ...
    <tbody>
      ...
      <tr>
        <td class="label-left">02 años</td>
  ...
  <!-- Sesiones Atención Temprana -->
  <div class="sub-section-title">II. SESIONES DE ATENCIÓN TEMPRANA (99411)</div>
  <table>
    <thead>
      <tr>
        <th class="th-dark">Edad</th>
        <th class="th-medium">1ª Sesión</th>
        <th class="th-medium">2ª Sesión</th>
        <th class="th-medium">3ª Sesión</th>
        <th class="th-medium">4ª Sesión</th>
        <th class="th-medium">5ª Sesión</th>
        <th class="th-dark">Niño con sesiones completas (Mensual)</th>
        <th class="th-dark">Niño con sesiones completas (Acum.)</th>
      </tr>
    </thead>
    <tbody>
  ...
  <table style="width:50%">
    <thead>
      <tr><th class="th-dark">CONDICIÓN</th><th class="th-dark">N°</th></tr>
    </thead>
    <tbody>
      ...
      <tr class="row-sub"><td class="label-left">Con alimentación mixta</td><td></td></tr>
    </tbody>
  </table>

  <!-- ===== XVI. IRAS / EDAS (desde FORMATO NIÑO) ===== -->
  <div class="section-title">XVI. ATENCIÓN DE LAS ENFERMEDADES PREVALENTES DE LA INFANCIA</div>

  <!-- A. IRA -->
  <div class="sub-section-title">A. INFECCIÓN RESPIRATORIA AGUDA (IRA)</div>
  <table>
    <colgroup>
      <col style="width:28%">
      <col style="width:10%">
    ...
    <thead>
      <tr>
        <th class="th-dark" rowspan="2">DIAGNÓSTICOS</th>
        <th class="th-dark" colspan="6">Grupo de Edad</th>
        <th class="th-dark" rowspan="2">Total</th>
      </tr>
      <tr>
        ...
        <th class="th-medium">02 - 11 Meses</th>
        <th class="th-medium">01 año</th>
        <th class="th-medium">2 año</th>
        <th class="th-medium">3 año</th>
        <th class="th-medium">4 año</th>
      </tr>
    </thead>
    <tbody>
      <tr class="row-header">
      ...
      <tr class="row-sub">
        <td class="label-left">1.1 N° casos de IRA sin complicaciones (a+b+c+d+e)</td>
      ...
      <tr><td class="label-indent">e. Neumonía sin complicaciones</td><td></td><td></td><td>44</td><td>26</td><td>25</td><td>23</td><td>205</td></tr>
      <tr class="row-sub">
        <td class="label-left">1.2 N° casos IRA con complicaciones (a+b+c)</td>
      ...
      <tr class="row-sub"><td class="label-indent">b. Neumonía Grave / EMG Menores de 2 Meses</td><td>1</td><td></td><td></td><td></td><td></td><td></td><td>1</td></tr>
      <tr><td class="label-indent">c. Neumonía y EMG en Niños de 2 Meses a 4 Años</td><td></td><td>16</td><td>20</td><td>14</td><td>7</td><td>3</td><td>60</td></tr>
    </tbody>
  </table>

  <!-- B. SOB -->
  <div class="sub-section-title">B. SÍNDROME DE OBSTRUCCIÓN BRONQUIAL (SOB) - ASMA</div>
  <table>
    <colgroup>
      <col style="width:28%">
      <col style="width:10%">
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td class="label-indent">a. Diarrea Aguda Acuosa sin deshidratación</td><td>758</td><td>949</td><td>502</td><td>351</td><td>255</td><td class="num">1,178</td><td class="num">3,993</td></tr>
      <tr class="row-sub"><td class="label-indent">b. Diarrea Aguda Disentérica sin deshidratación</td><td>9</td><td>13</td><td>9</td><td>9</td><td>8</td><td>29</td><td>77</td></tr>
      <tr><td class="label-indent">c. Diarrea Persistente sin deshidratación</td><td>469</td><td>549</td><td>280</td><td>202</td><td>124</td><td>548</td><td class="num">2,172</td></tr>
      <tr class="row-header">
      ...
      <tr><td class="label-indent">a. Diarrea Aguda Acuosa con deshidratación</td><td>12</td><td>36</td><td>5</td><td>2</td><td>3</td><td>12</td><td>70</td></tr>
      <tr class="row-sub"><td class="label-indent">b. Diarrea Aguda Disentérica con deshidratación</td><td>6</td><td>14</td><td>1</td><td>2</td><td>1</td><td class="zero">0</td><td>24</td></tr>
      <tr><td class="label-indent">c. Diarrea Persistente con deshidratación</td><td>6</td><td>14</td><td>1</td><td>2</td><td>1</td><td>6</td><td>30</td></tr>
      <tr class="row-sub"><td class="label-indent">d. Diarrea Aguda Acuosa con deshidratación con shock</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td></tr>
      <tr><td class="label-indent">e. Diarrea Aguda Disentérica con deshidratación con shock</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td></tr>
      <tr class="row-sub"><td class="label-indent">f. Diarrea Persistente con deshidratación con shock</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td></tr>
    </tbody>
  </table>

  <!-- Zinc y SRO -->
  <div class="sub-section-title">ADMINISTRACIÓN DE ZINC Y SAL DE REHIDRATACIÓN ORAL</div>
  <table>
    <colgroup>
      <col style="width:28%">
      <col style="width:11%">
    ...
    <tbody>
      <tr>
        <td class="label-left">Administración de tratamiento - Sales de Rehidratación Oral (SRO)</td>
        <td>10</td><td>6</td><td>7</td><td>1</td><td>2</td><td>4</td><td class="zero">0</td>
      </tr>
      <tr class="row-sub">
        <td class="label-left">Administración de tratamiento - Zinc (ZN)</td>
...
## 23. REFERENCIA HTML - foramto exel iras_edas.txt
...
<div class="page">
  ...
  <!-- Page title -->
  <div class="page-number">Página 06</div>
  <div class="section-title">XVI. ATENCIÓN DE LAS ENFERMEDADES PREVALENTES DE LA INFANCIA</div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr class="row-sub">
        <td class="bold" colspan="2" style="text-align:left;padding-left:6px;">1.1 N° casos de IRA sin complicaciones (a+b+c+d+e)</td>
        <td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">a. Infección Respiratoria Aguda (IRA) no complicada</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">e. Neumonía sin complicaciones</td>
      ...
      <tr class="row-sub">
        <td class="bold" colspan="2" style="text-align:left;padding-left:6px;">1.2 N° casos IRA con complicaciones (a+b+c)</td>
        <td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td><td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">a. Infecciones Respiratorias Agudas con complicaciones</td>
        <td class="formula">TD=D+DX=A369, A370, A371, A378, A379, J120, J121, J122, J123, J128, J13X, J14X, J150…</td>
        <td class="formula">TD=D+DX=A369, A370, A371, A378, A379, J120, J121, J122, J123, J128…</td>
        <td class="formula">TD=D+DX=A369, A370, A371, A378, A379, J120, J121, J122, J123, J128…</td>
        <td class="formula">TD=D+DX=A369, A370, A371, A378, A379, J120, J121, J122, J123, J128…</td>
        <td class="formula">TD=D+DX=A369, A370, A371, A378, A379, J120, J121, J122, J123, J128…</td>
        <td class="zero">0</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">b. Neumonía Grave o Enfermedad Muy Grave en Niños Menores de 2 Meses</td>
        <td class="formula">TD=D+DX=J050, J051, J851, J860, J869, J90X, J939, J100, J110…</td>
        <td class="formula">TD=D+DX=J050, J051, J851, J860, J869, J90X, J939, J100, J110…</td>
        <td></td><td></td><td></td>
        <td class="zero">0</td>
        <td class="zero">0</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">c. Neumonía y Enfermedad Muy Grave en Niños de 2 Meses a 4 Años</td>
        <td></td><td></td>
        <td class="formula">TD=D+DX=J050, J051, J851, J860, J869, J90X, J939, J100, J110…</td>
        <td class="formula">TD=D+DX=J050, J051, J851, J860, J869, J90X, J939, J100, J110…</td>
        <td></td>
  ...
  <!-- Oxigenoterapia -->
  <div class="sub-section-title">OXIGENOTERAPIA / OXIMETRÍA</div>
  <table>
    <colgroup>
      <col style="width:5%">
      <col style="width:22%">
    ...
    <tbody>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">Oximetría</td>
  ...
  <table>
    ...
    <thead>
      ...
      <tr>
        ...
        <th class="th-medium">02 - 02 Años 11m</th>
        <th class="th-medium">03 - 04 Años</th>
        <th class="th-medium">05 - 11 Años</th>
      </tr>
    </thead>
    <tbody>
  ...
  <!-- Oxigenoterapia y Nebulización -->
  <div class="sub-section-title">OXIGENOTERAPIA Y NEBULIZACIÓN</div>
  <table>
    <colgroup>
      <col style="width:5%">
      <col style="width:19%">
    ...
    <tbody>
      <tr>
        <td></td>
        <td class="diag-sub">Oxigenoterapia</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94799.02)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94799.02)</td>
        <td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">Nebulización / Inhaloterapia</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J210…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94664)</td>
        <td class="formula">(TD=R+DX=(J440…J46X)+DX=94664)</td>
        <td class="zero">0</td>
      </tr>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">a. Diarrea Aguda Acuosa sin deshidratación</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">b. Diarrea Aguda Disentérica sin deshidratación</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">c. Diarrea Persistente sin deshidratación</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">a. Diarrea Aguda Acuosa con deshidratación</td>
        ...
        <td class="formula">TD=D+DX=A00.9+E86X,A01.0+E86X,A01.1+E86X…A09.9+E86X</td>
        <td class="formula">TD=D+DX=A00.9+E86X,A01.0+E86X,A01.1+E86X…A09.9+E86X</td>
        <td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">b. Diarrea Aguda Disentérica con deshidratación</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">c. Diarrea Persistente con deshidratación</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">d. Diarrea Aguda Acuosa con deshidratación con shock</td>
        <td class="formula">TD=D+DX=A00.9+E86X+R57.1,A01.0+E86X+R57.1…A09.9+E86X+R57.1,A049+E86X+R57.1</td>
        <td class="formula">TD=D+DX=A00.9+E86X+R57.1…A09.9+E86X+R57.1</td>
        <td class="formula">TD=D+DX=A00.9+E86X+R57.1…A09.9+E86X+R57.1</td>
        <td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">e. Diarrea Aguda Disentérica con deshidratación con shock</td>
        ...
        <td class="formula">TD=D+DX=A030+E86X+R57.1…A060+E86X+R57.1</td>
        <td class="formula">TD=D+DX=A030+E86X+R57.1…A060+E86X+R57.1</td>
        <td class="zero">0</td>
      </tr>
      <tr>
        <td></td>
        <td class="diag-sub">f. Diarrea Persistente con deshidratación con shock</td>
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td></td>
        <td class="diag-sub">Administración de tratamiento (Sales de Rehidratación Oral - SRO)</td>
      ...
      <tr>
        <td></td>
        <td class="diag-sub">Administración de tratamiento (Zinc - ZN)</td>
  ...
  <table>
    ...
    <tbody>
      <tr><td>1 - Enero</td><td>144</td><td>174</td><td>110</td><td>57</td><td>43</td><td>2030</td><td>—</td><td>40</td><td>25</td><td>25</td><td>17893</td></tr>
      <tr class="row-sub"><td>2 - Febrero</td><td>134</td><td>155</td><td>67</td><td>59</td><td>41</td><td>1895</td><td>—</td><td>34</td><td>28</td><td>28</td><td>17536</td></tr>
      <tr><td>3 - Marzo</td><td>142</td><td>197</td><td>104</td><td>82</td><td>66</td><td>2432</td><td>—</td><td>37</td><td>22</td><td>22</td><td>20818</td></tr>
      <tr class="row-sub"><td>4 - Abril</td><td>146</td><td>171</td><td>98</td><td>79</td><td>56</td><td>3611</td><td>—</td><td>48</td><td>35</td><td>35</td><td>20866</td></tr>
      <tr><td>5 - Mayo</td><td>160</td><td>216</td><td>104</td><td>66</td><td>42</td><td>4212</td><td>—</td><td>69</td><td>52</td><td>52</td><td>21208</td></tr>
      <tr class="row-sub"><td>6 - Junio</td><td>32</td><td>36</td><td>19</td><td>8</td><td>7</td><td>1100</td><td>—</td><td>21</td><td>27</td><td>27</td><td>6422</td></tr>
      <tr style="background:#2E75B6;color:#fff;font-weight:bold;">
        <td>TOTAL GENERAL</td><td>758</td><td>949</td><td>502</td><td>351</td><td>255</td><td>15280</td><td>—</td><td>249</td><td>189</td><td>189</td><td>104743</td>
      </tr>
    </tbody>
  </table>

  <div style="font-size:8px;color:#595959;margin-top:8px;border-top:1px solid #9DC3E6;padding-top:4px;">
    DIRESA CUSCO &nbsp;|&nbsp; DEIT &nbsp;|&nbsp; Sistema HIS &nbsp;|&nbsp; Año 2026 &nbsp;|&nbsp; Impreso: <span id="fecha"></span>
  </div>
</div>
<script>
  document.getElementById('fecha').textContent = new Date().toLocaleDateString('es-PE');
